...
insert overwrite table edu_dwd.dwd_web_chat_ems_fact partition (yearinfo, quarterinfo, monthinfo, dayinfo)
select id,
       create_date_time,
       session_id,
       sid,
...
insert overwrite table edu_dwd.dwd_web_chat_text_ems_fact partition (dt)
select id,
       referrer,
       from_url,
       landing_page_url,
